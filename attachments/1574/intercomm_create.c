...
/* 1024 allows us to handle up to 32k processes */
#ifdef HAVE_ERROR_CHECKING
#define MAX_LPID32_ARRAY 1024
#undef FUNCNAME
...
#if !defined(_CRAY_VECTOR)  && !defined(_CRAY_XT3_LINUX)
...
#       endif /* HAVE_ERROR_CHECKING */
	...
	*/
	...
	/* Exchange the lpid arrays */
	mpi_errno = MPIC_Sendrecv( local_lpids, local_size, MPI_INT, 
		       remote_leader, tag,
...
#       endif /* HAVE_ERROR_CHECKING */
	...
	mpi_errno = MPIR_Bcast_impl( remote_gpids, 2*remote_size, MPI_INT, local_leader,
                                     comm_ptr );
#else
	mpi_errno = MPIR_Bcast_impl( remote_lpids, remote_size, MPI_INT, local_leader,
                                     comm_ptr );
#endif
        if (mpi_errno) MPIU_ERR_POP(mpi_errno);
