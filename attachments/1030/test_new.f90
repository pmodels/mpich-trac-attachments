...
program test
  ...
  integer :: aext(2), astart, aend, dt_size, i, j, size_exp
  integer, parameter :: cnt = 2
  integer :: blens(cnt), types(cnt)
  integer (kind=MPI_ADDRESS_KIND) :: displ(cnt), lb, dt_ext
  ...
  ! create type
  call MPI_Type_create_struct(cnt, blens, displ, types, datatype, err)
  call MPI_Type_commit(datatype, err)
  call MPI_Type_size(datatype, dt_size, err)
  call MPI_Type_get_extent(datatype, lb, dt_ext, err)
...
contains
  ...
  subroutine build_array_section_type(aext,astart,aend,datatype)
    ...
    integer (kind=MPI_ADDRESS_KIND) :: displ(count)
    ...
    call MPI_Type_create_struct(count, blens, displ, types, datatype, err)
