 ...
 * when ran on a lustre filesystem, only the first 2GB are written.
 * 
...
int main (int argc, char *argv[]) {
...
#ifndef STATUS_IGNORE
    if (MPI_File_write(fd, array, N, MPI_DOUBLE, &status) != MPI_SUCCESS)
#else
    if (MPI_File_write(fd, array, N, MPI_DOUBLE, MPI_STATUS_IGNORE) != MPI_SUCCESS)
#endif
    {
    ...
    if (lseek(filedes, file_size/2, SEEK_SET) < 0) {
        perror ("lseek ");
        MPI_Abort(MPI_COMM_WORLD, 9);
    }

    if (read(filedes, &d, sizeof(double)) != sizeof(double)) {
        perror ("read ");
        MPI_Abort(MPI_COMM_WORLD, 10);
    }

    if (d != (N/2) ) {
        fprintf(stderr, "got %lf but expected %d\n", d, N/2);
