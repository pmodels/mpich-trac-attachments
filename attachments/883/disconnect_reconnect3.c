 ...
 */
void checkReverseMergedComm( MPI_Comm comm, int size ) {
    MPI_Group origGroup;
    MPI_Group newGroup;
    MPI_Comm newComm;
    int * newGroupRanks = (int *)calloc( size, sizeof( int ) );
    int ii;
    for ( ii = 0; ii < size; ++ii ) {
        newGroupRanks[ii] = size - ii - 1;
    }
    MPI_Comm_group( comm, &origGroup );
    MPI_Group_incl( origGroup, size, newGroupRanks, &newGroup );
    free( newGroupRanks );
...
int main(int argc, char *argv[])
{
    ...
    checkReverseMergedComm( intracomm, np );
